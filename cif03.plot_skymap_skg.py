...
ax = fig.add_subplot(111, projection="mollweide") # Use #2D2D2D for black
plt.grid(True)
...
for i in range(len(SKG_list01)):
    sc01 = ax.scatter(x01[i], y01[i], s=25, c=z01[i], label = SKG_list01[i], zorder=1)
    sc02 = ax.scatter(x02[i], y02[i], s=25, c=z02[i], label = SKG_list02[i], zorder=1)
