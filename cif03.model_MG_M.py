...
df = df.dropna(subset=['eM_Msol', 'M_Msol', 'MG_mag', 'eMG_mag'])  

filter_OK = (df['Bool_MM'] == True) 
filter_single = (df['Type'] == 'Single') # Single stars (avoiding unresolved binaries too)
filter_Teff = (df['Teff_ref'] == 'Marf21') # Only spectroscopic Teff

type = 'M'
MG_max = 14

MG_mag = df[filter_single & filter_Teff & filter_OK]['MG_mag']
eMG_mag = df[filter_single & filter_Teff & filter_OK]['eMG_mag']
x, ex = MG_mag, eMG_mag
x_fit, ex_fit = MG_mag[MG_mag < MG_max], eMG_mag[MG_mag < MG_max]

if type == 'M':
    output_file = 'plot_M_MG'
    M_Msol = df[filter_single & filter_Teff & filter_OK]['M_Msol']
    eM_Msol = df[filter_single & filter_Teff & filter_OK]['eM_Msol']
    y, ey = M_Msol, eM_Msol
    y_fit, ey_fit = M_Msol[MG_mag < MG_max], eM_Msol[MG_mag < MG_max]
elif type == 'R':
    output_file = 'plot_R_MG'
    R_Rsol = df[filter_single & filter_Teff & filter_OK]['R_Rsol']
    eR_Rsol = df[filter_single & filter_Teff & filter_OK]['eR_Rsol']
    y, ey = R_Rsol, eR_Rsol
...
if plot:
    ...
    ax[0].scatter(x, y, facecolors='white', edgecolors='black', s=pointsize, zorder=1)
    ax[0].errorbar(x, y, xerr=ex, yerr=ey, ls='none', ecolor='black', zorder=0)
    ...
    #
    ax[1].scatter(x, OC, facecolors='none', edgecolors='black', s=pointsize, zorder=0)
    ...
    if type == 'M':
        ...
        ax[1].set_ylabel(f'O-C 'r'[$\mathcal{R}_\odot]$', size=labelsize*0.8)
    ax[1].set_xlim(7.5, 14)
    ax[1].set_ylim(-.12, .12)
    #
    if save_plot == True:
...
save_csv, output_full = [False, False]
