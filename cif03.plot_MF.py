...
def MF_distance(df, distance, SpTnum):
    """
    Calculates the number of stars within a specified distance from a reference point in a dataset, considering certain criteria. 

    1. `df`: The input DataFrame containing information about stars.
    2. `distance`: The maximum distance in parsecs within which stars are considered.
    3. `SpTnum`: The spectral type number, where the function filters stars with spectral types matching `SpTnum` or `SpTnum + 0.5`.
    """
    filter_SpT = ((df['SpTnum'] == SpTnum) | (df['SpTnum'] == SpTnum+0.5))
...
MF_SpT = [MF_CSF_SpT(df, n)[0] for n in np.arange(70, 77, 1)]
CSF_SpT = [MF_CSF_SpT(df, n)[1] for n in np.arange(70, 77, 1)]
...
if plot == 'MF_dist':
    ...
    # plt.bar(x+0.2, CSF_SpT, width=0.4, bottom=None, align='center', fill = False, edgecolor = 'red', linewidth=linewidth, hatch="/")
    #
    plt.xticks(np.arange(70, 77, 1))
